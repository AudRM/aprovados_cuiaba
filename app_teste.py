...
Arquivo contendo a lógica de controle e de apresentação do sistema.
...
import pandas as pd 
from utils import encriptar_arquivo, decriptar_arquivo
import os
    ...
    with st.form("Criar Conta"):
        ...
        if submit:
            ...
            if not documento:
             ...
             # Carrega a chave de criptografia (ajustar caminho, etc.)
            caminho_chave = "chave_fernet.key"
            if not os.path.exists(caminho_chave):
                st.error("Chave de criptografia não encontrada. Contate o administrador.")
                return
            
            chave = carregar_chave_criptografia(caminho_chave)
            ...
            # Salva o arquivo criptografado em uma pasta protegida
            ...
            nome_arquivo = f"{n_inscr}_{timestamp}.{extensao}.enc"  # por exemplo
            caminho_arquivo = os.path.join(pasta_destino, nome_arquivo)

            with open(caminho_arquivo, "wb") as f:
                f.write(conteudo_criptografado)

            # Processar a criação da conta normalmente
...
def administrar_web_app():
    ...
    if st.button("Ver Arquivo"):
        ...
        # Lista todos os arquivos naquela pasta e filtra pelos que contêm o n_inscr
        arquivos = [f for f in os.listdir(pasta_destino) if n_inscr_arquivo in f]
        
        if arquivos:
            st.success(f"Arquivo(s) encontrado(s): {arquivos}")
            arquivo_escolhido = arquivos[0]  # Pega o primeiro, ou poderia ter um selectbox
            ...
            # Agora podemos oferecer um botão de download do conteúdo decriptado
            # Usamos o mesmo nome do arquivo original (sem o .enc)
